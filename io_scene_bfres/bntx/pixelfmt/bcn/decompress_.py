...
def dxt5_alphablock_signed(data, blksrc):
    ...
    if to_signed_8(alpha[0]) > to_signed_8(alpha[1]):
        for i in range(2, 8):
            alpha[i] = to_unsigned_8((to_signed_8(alpha[0]) * (8 - i) + (to_signed_8(alpha[1]) * (i - 1))) // 7)
    else:
        for i in range(2, 6):
            alpha[i] = to_unsigned_8((to_signed_8(alpha[0]) * (6 - i) + (to_signed_8(alpha[1]) * (i - 1))) // 5)
        alpha[6] = 0x80
        alpha[7] = 0x7F
    return bytes(alpha)
        ...
        for x in range(w):
            ...
            for ty in range(th):
                for tx in range(tw):
                    ...
                    if snorm:
                        output[ooffs + 0] = to_signed_8(R[(RedCh >> shift) & 7]) + 0x80
                    else:
        ...
        for x in range(w):
            ...
            for ty in range(th):
                for tx in range(tw):
                    ...
                    if snorm:
                        output[ooffs] = to_signed_8(R[(RedCh >> shift) & 7]) + 0x80
                        output[ooffs + 1] = to_signed_8(G[(GreenCh >> shift) & 7]) + 0x80
                    else:
