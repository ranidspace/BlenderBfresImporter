...
class ShapeParser:
    ...
    def read(loader: ResFileSwitchLoader, shape: Shape):
        ...
        if radius_offs != 0 and num_mesh > 0:
            with loader.temporary_seek(radius_offs, io.SEEK_SET):
                if loader.res_file.version_major2 >= 10:
                    ...
                    num_boundings = num_mesh if num_skin_bone_idx == 0 else num_skin_bone_idx
                    for i in range(num_boundings):
                        shape.bounding_radius_list.append(loader.read_vector4f())
        ...
        shape.meshes = [] if num_mesh == 0 else loader.load_list(Mesh, num_mesh, mesh_array_offs)
        shape.skin_bone_idxs = (
            ()
            if num_skin_bone_idx == 0
            else loader.load_custom(tuple, lambda: loader.read_uint16s(num_skin_bone_idx), skin_bone_idx_list_offs)
        )

        bounding_box_cnt = sum([len(mesh.submeshes) + 1 for mesh in shape.meshes])
        shape.submesh_boundings = (
            []
            if bounding_box_cnt == 0
            else loader.load_custom(list, lambda: loader.read_boundings(bounding_box_cnt), bounding_box_array_offs)
        )
