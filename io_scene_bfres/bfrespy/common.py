...
class Node(Generic[T]):
    ...
    def __init__(self, key: str | None = None, value: T | None = None):
...
class ResDict(core.ResData, Collection[Node[T]]):
    ...
    def __contains__(self, x: object) -> bool:
        node, _ = self.__lookup(x, throwonfail=False)
        return bool(node)
    ...
    def __getitem__(self, key: int | str | type[core.ResData]):
        if isinstance(key, int | str):
            node, _ = self.__lookup(key)
            return node.value

        if isinstance(key, core.ResData):
            node, _ = self.__lookup(key)
            return node.key
        msg = f'Invalid type "{type(key).__name__}"'
    ...
    def key_index(self, key: str):
        """Search for the specified key and returns the zero-based
        index of the first occurrence within the entire dictionary.
        """
        _, index = self.__lookup(key, throwonfail=False)
        return index

    def try_get_value(self, key):
        node, _ = self.__lookup(key, throwonfail=False)
        if node:
            return node.value
        return None
    ...
    def value_index(self, value: core.ResData):
        """Search for the specified value and return the zero-based
        index of the first occurrence within the entire dictionary.
        """
        node, index = self.__lookup(value, throwonfail=False)
        if node:
            return index
        return -1

    def get_key(self, index: int):
        return self._nodes[index + 1].key

    def key_from_value(self, value) -> None:
        """Return True if a key was found for the given value and has been
        assigned to key, or None if no key was found.
        """
        node, _ = self.__lookup(value, throwonfail=False)
        if node:
            return node.key
        return None
    ...
    def load(self, node_type: type[T], loader: core.ResFileLoader):
        ...
        nodes: list[Node[T]] = []
        while num_nodes >= 0:
            nodes.append(self.__read_node(node_type, loader))
    ...
    @staticmethod
    def _load_node_value(node_type: type[T], loader):
        """Load a ResData instance from the given loader"""
        return loader.load(node_type)

    # Private Methods

    def __read_node(self, node_type: type[T], loader: core.ResFileLoader) -> Node[T]:
        node = Node()
        ...
        if not loader.is_switch:
            node.value = self._load_node_value(node_type, loader)
        return node
    ...
    def _(self, value: ResString, throwonfail=True):
        for i, found_node in enumerate(self):
            if isinstance(found_node.value, ResString) and str(found_node.value) == str(value):
                return (found_node, i)
        return (Node(), -1)
    ...
    def _(self, key: int, throwonfail=True):
        if key < 0 or key > len(self._nodes):
            if throwonfail:
                msg = f"{key} out of bounds in {self}."
